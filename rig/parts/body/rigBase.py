...
class BASERIG(BASECHAIN):
	def __init__(self, items, *args, **kwargs):
		BASECHAIN.__init__(self, items=items, *args, **kwargs)
	...
	def create3PointFKIK(self, items=None, parent=None):
		...
		if parent is None:
			if self.master:
				...
				self.master = self.createMaster(items[-1])
				parent = self.master
	...
	def createFKIKConnections(self, items=None, parent=None):
		...
		for ctrl in self.fkControl:
			cmds.connectAttr('{}.outputX'.format(fkik[1][i]), '{}.v'.format(ctrl.nullPosition), f=True)
			i += 1

		for ctrl in self.ikControl:
			cmds.connectAttr('{}.{}'.format(parent, Component.fkik), '{}.v'.format(ctrl.nullPosition), f=True)
		return

	@staticmethod
	def connectToMaster(child, master, name):
		attrName = Component.master
		childAttr = '{}.{}'.format(child, attrName)
		masterAttr = '{}.{}'.format(master, name)

		if not attributeExist(master, name):
			cmds.addAttr(master, ln=name, at='message')

		if not attributeExist(child, attrName):
			cmds.addAttr(child, ln=attrName, at='message')

		if cmds.attributeQuery(name, node=master, ex=True):
			if cmds.attributeQuery(name, node=master, m=True):
				mList = cmds.listAttr('{}.{}'.format(master, name), m=True)
				if mList:
				...
				masterAttr = '{}.{}[{}]'.format(master, name, i)

		cmds.connectAttr(masterAttr, childAttr, force=True)
		return

	@staticmethod
	def connectItemToNetworkRoot(item, networkRoot):
		attrName = 'rigNetworkRoot'
		if not cmds.attributeQuery(attrName, node=item, ex=True):
			cmds.addAttr(item, ln=attrName, at='message')
		if not getConnectedNode(item, attrName):
	...
	def finalize(self, items=None, parent=None):
		...
		# Set
		if self.set:
			self.connectToMaster(self.set, parent, Component.set)

		# FK
		if self.fkControl:
			self.multiConnectToNetwork(self.fkControl, parent, Component.fkControl)

		if self.fkPoleVector:
			self.connectToMaster(self.fkPoleVector, parent, Component.fkPoleVector)

		# IK
		if self.ikControl:
			self.multiConnectToNetwork(self.ikControl, parent, Component.ikControl)

		if self.ikJoint:
			self.multiConnectToNetwork(self.ikJoint, parent, Component.ikJoint)

		if self.ikPoleVector:
			self.connectToMaster(self.ikPoleVector, parent, Component.ikPoleVector)

		if self.ikHandle:
			self.connectToMaster(self.ikHandle, parent, Component.ikHandle)
...
class RIGBASE(object):
	...
	def createMaster(self, child=None, wireType=None):
		...
		ctl = MASTERCONTROL(name=longName(self.name,
		                                  self.side.upper()[0],
		                                  self.index,
		                                  Component.attr,
		                                  Component.control,
