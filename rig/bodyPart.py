...
class BASE(object):
		...
		for obj in objects:
			...
			if children:
				for child in children:
					ctl = rig.api.component.DETAILCONTROL(name=naming.convention(self.name,
					                                                             self.side.upper()[0],
					                                                             self.index,
					                                                             naming.rig.detail,
					                                                             indexNum,
					...
					if grandchildren:
						...
						for grandchild in grandchildren:
							gctl = rig.api.component.GRANDCHILDCONTROL(name=naming.convention(grandchild,
							                                                                  naming.rig.ctl
							                                                                  ),
			...
			if children:
				for child in children:
					...
					if grandchildren:
						...
						for grandchild in grandchildren:
							ctl = rig.api.component.GRANDCHILDCONTROL(name=naming.convention(grandchild,
							                                                                 naming.rig.ctl
							                                                                 ),
							                                          scale=cmds.getAttr('{}.radius'.format(grandchild)),
							                                          parent=None,
	...
	def createRibbonChain(self, start, mid, end):
		...
		if upperObjects and lowerObjects:
			...
			ribbon = rig.api.component.RIBBONLIMB(name=naming.convention(self.name,
			                                                             self.side[0].upper(),
			                                                             self.index,
			                                                             naming.rig.ribbon,
			                                                             ),
	...
	def createTwistChain(self, start, mid, end):
		chain = rig.api.component.TWISTCHAIN(start=start,
		                                     mid=mid,
		                                     end=end,
		                                     scale=self.scale,
		                                     axis=self.axis,
	...
	def createFKChain(self, objects):
		...
		fk = rig.api.component.FKCHAIN(objects,
		                               name=name,
		                               scale=self.scale,
		                               axis=self.axis,
		                               side=self.side,
	...
	def createIKChain(self, objects):
		...
		ik = rig.api.component.IKCHAIN(objects,
		                               name=name,
		                               scale=self.scale,
		                               axis=self.axis,
		                               side=self.side,
	...
	def createAttrControl(self, obj):
		...
		if self.name == 'leg':
			if self.side == 'Left':
				...
				axis = [1, 0, -1]
		'''
		ctl = rig.api.component.ATTRCONTROL(name=naming.convention(self.name,
		                                                           self.side.upper()[0],
		                                                           self.index,
		                                                           naming.rig.attr,
		                                                           naming.rig.ctl,
