...
def createFKIK(items, fkControls, ikControls, master, attrName=Component.fkik):
	# Add Attribute

	if not attributeExist(master, attrName):
		cmds.addAttr(master, ln=attrName, min=0, max=1, dv=0, k=True)

	# Error Checks

	items = listCheck(items)
	fkControls = listCheck(fkControls)
	ikControls = listCheck(ikControls)
	...
	for item in items:
		if len(items) == 1:
			pc = cmds.parentConstraint(fkControls,
			                           ikControls,
			                           item,
		...
		cmds.connectAttr('{}.{}'.format(master, attrName), '{}.{}'.format(pc, pcAttr[-1]), f=True)

		reverse = cmds.createNode('reverse', n='{}_{}_re1'.format(item, attrName))
		cmds.connectAttr('{}.{}'.format(master, attrName), '{}.inputX'.format(reverse), f=True)
