...
class BASECHAIN(object):
	...
	def allAnimationControls(self):
		...
		if self.master:
			result.append(self.master)

		for x in [self.fkControl, self.ikControl, self.bindControls, self.leafControls]:
	...
	def createFKChain(self, items=None, parent=None):
		...
		# FK PoleVector
		if len(items) == 3:
			self.fkPoleVector = self.createFKPoleVector(items)
	...
	def createFKPoleVector(self, items):
		fkPoleVector = cmds.group(n=longName(self.longName, Component.fkPoleVector), em=True)
		pvPos = getPoleVectorPosition(items[0], items[1], items[2])
		cmds.xform(fkPoleVector, ws=True, t=pvPos)
		return fkPoleVector
	...
	# TODO: Bind Chain
	def createBindChain(self):
		return

	# TODO: Leaf Chain
	def createLeafChain(self):
		return

	# TODO: Twist CHAIN
	def createTwistChain(self):
		return
	...
	def createMaster(self, child=None, wireType=WireType.master):
		axis = [0, 0, 0]
		...
		ctl = MASTERCONTROL(name=self.name,
		                    prefix=self.prefix,
		                    item=child,
		                    axis=axis,
		                    scale=self.scale,
...
class CHAIN(object):
	...
	def createParent(self):
		self.parent = createGroup(self.group[0], name=longName(self.name, Component.group))
		return
...
class FKCHAIN(CHAIN):
	...
	def createStretch(self):
		...
		for ctl in self.control:
			...
			grp = createGroup(ctl, name=longName(ctl, attrName, Component.group))
...
class IKCHAIN(CHAIN):
	...
	def createJointChain(self):
		...
		self.joint = createJointChain(self.items, name=name)
...
class DETAILCHAIN(CHAIN):
	def __init__(self,
		...
		CHAIN.__init__(self,
		               items=getBindJoint(joint),
		               controlClass=BINDCONTROL,
		               name=name,
		               scale=scale,
		               axis=axis,
		               side=getSide(joint),
